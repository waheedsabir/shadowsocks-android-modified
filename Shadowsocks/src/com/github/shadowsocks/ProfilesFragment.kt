...
class ProfilesFragment : ToolbarFragment(), Toolbar.OnMenuItemClickListener {
    ...
    class QRCodeDialog() : DialogFragment() {
        ...
        override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
            ...
            // XinLake. Replaced with ZXing
            val bitmap = Encoder.EncodeAsBitmap(url, BarcodeFormat.QR_CODE, size)
    ...
    inner class ProfileViewHolder(view: View) : RecyclerView.ViewHolder(view), View.OnClickListener,
        ...
        override fun onMenuItemClick(item: MenuItem): Boolean = when (item.itemId) {
            R.id.action_qr_code -> {
                parentFragmentManager.beginTransaction().add(QRCodeDialog(this.item.toString()), "").commitAllowingStateLoss()
                true
            }
    ...
    inner class ProfilesAdapter : RecyclerView.Adapter<ProfileViewHolder>(), ProfileManager.Listener {
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ProfileViewHolder =
            ProfileViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.layout_profile, parent, false))
    ...
    private fun startConfig(profile: Profile) {
        ...
        startActivity(Intent(context,
            ProfileConfigActivity::class.java).putExtra(Action.EXTRA_PROFILE_ID,
            profile.id))
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        /* XinLake. Empty is OK */
        if (ProfileManager.getAllProfiles()!!.isEmpty() && context != null) {
            MaterialDialog(context!!).show {
                title(R.string.add_first_profile)
                listItems(R.array.add_first_profile) { _, index, _ ->
                    when (index) {
                        0 -> startActivity(Intent(context, ScannerActivity::class.java))
                        1 -> startFilesForResult(Intent(Intent.ACTION_GET_CONTENT).apply {
                            type = "application/*"
                            putExtra(Intent.EXTRA_ALLOW_MULTIPLE, true)
                            putExtra(Intent.EXTRA_MIME_TYPES, arrayOf("application/*", "text/*"))
                        }, REQUEST_IMPORT_PROFILES)
                        2 -> startConfig(ProfileManager.createProfile(Profile().also {
                            Core.currentProfile?.first?.copyFeatureSettingsTo(it)
                        }))
                    }
                }
            }
        ...
        ItemTouchHelper(object : ItemTouchHelper.SimpleCallback(ItemTouchHelper.UP or ItemTouchHelper.DOWN, ItemTouchHelper.START or ItemTouchHelper.END) {
            override fun getSwipeDirs(recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder): Int =
                if (isProfileEditable((viewHolder as ProfileViewHolder).item.id)) super.getSwipeDirs(recyclerView, viewHolder)
                else 0
    ...
    override fun onMenuItemClick(item: MenuItem): Boolean {
        return when (item.itemId) {
            ...
            R.id.action_import_clipboard -> {
                try {
                    val profiles = Profile.findAllUrls(clipboard.primaryClip!!.getItemAt(0).text, Core.currentProfile?.first).toList()
        ...
        if (resultCode != Activity.RESULT_OK) super.onActivityResult(requestCode, resultCode, data)
            ...
            REQUEST_EXPORT_PROFILES -> {
                ...
                if (profiles != null) try {
                    requireContext().contentResolver.openOutputStream(data?.data!!)!!.bufferedWriter()
                        .use {
